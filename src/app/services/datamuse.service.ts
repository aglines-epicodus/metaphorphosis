...
export class DatamuseService {
  ...
  getNounsWithBetterFrequency(noun: string) {
    return this.http.get(`http://api.datamuse.com/words?rel_trg=${noun}&md=f&max=25`);
  }
  ...
  getDatamuseResponse() {
    ...
    var frequency = this.http.get("http://api.datamuse.com/words?f=" + selectedWord + "&max=25").subscribe((result) => {
      for (var i=0; i<result.json().length; i++) {
        console.log("frequency ", selectedWord," = ", result.json()[i].word);
      }
    });
